...
class contentlistserver(TCPNetworkHandler):
    ...
    def packet_getpkgcs(self, clientid, islan):
        ...
        if all_count > 0:
            ...
            for ip, port in all_results:
                log.debug(f"{clientid}pkg update servers: {ip} {port}")
    ...
    def packet_getcswithapps(self, clientid, msg, islan):
        ...
        results, count = manager.find_ip_address(str(region), str(appnum), str(version), islan = islan)
