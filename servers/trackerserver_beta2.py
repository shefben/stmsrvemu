...
from utilities.database.betatrackerdb import beta2_dbdriver
...
class Message:
    ...
    def getpacket(self):
        logger.debug("preparing packet with id %d  clientid  %d  sessionid %d  seqnum %d  replyto %d" % (self.cmdid, self.client.clientid, self.sessionid, self.seqnum, self.reply_to))
...
network_key = (# n
        , # e
        0x11, # d
...
class TrackerServer(UDPNetworkHandler):
    ...
    def handle_client(self, data, addr):
        ...
        clientid = str(addr) + ": "
        logger.info(clientid + "Connected to Tracker Server")
            ...
            for curr_attempt, sched_time, msg in self.pending:
                ...
                if msg.seqnum > msg.client.ackd:
                    if sched_time < curr_time:
                        ...
                        data = msg.getpacket()
                        self.serversocket.sendto(data, msg.client.addr)
    ...
    def handle_msg(self, client, pkt):
        ...
        if cmdid == 2001:
            ...
            if client.sessionid != pkt.sessionid:
                logger.error(f"SessionID Does not match! {client.sessionid} != {pkt.sessionid}")  # raise Exception("bad sessionid", client.sessionid, pkt.sessionid)
        ...
        if cmdid == 2001:  # pre-login
            ...
            if client.status == 4:  # ingame
                ...
                logger.info("uid %x is playing a game: %s on ip %s port %s" % (client.uid, client.game, socket.inet_ntoa(client.gameip), client.gameport.hex()))
            ...
            logger.warning("forwarding message encountered id %d  uid %x  sessionid %d  serverid %d" % (rid, ruserid, rsessionid, rserverid))
            ...
            if "AddOnFlag" in msg and di(msg["AddOnFlag"]) == 1:
                ...
                logger.info("user %x sent addon message targetid %x  gameid %d  sessionid %d  msg size %d" % (uid, targetid, gameid, addonsess, msgdatalen))
                ...
                logger.info("user %x with username %s sent a message targetid %x  status %x  text %s" % (uid, username, targetid, status, text))
            ...
            logger.warning("user %x tried to update block status for user %x but we don't support it" % (client.uid, uid))
            ...
            logger.info("user %x is typing, state %x  chatid %x  status %x  target %x" % (source, state, chatid, status, target))
