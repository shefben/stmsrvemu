...
class MasterServer(UDPNetworkHandler):
    ...
    def packet_heartbeat(self, data, address):
        ...
        if not server:
            ...
            server = Server(ds_ip, False, False, int(time.time()), self.last_unique_id + 1, False, False, "", int(num), 0, 0, gamedir, "", "w", False, False, False, info, len(info))
    ...
    def packet_heartbeat2(self, data, address):
        ...
        if server is None:  # Check if the server was not found
            ...
            server = Server(ds_ip, False, False, 0, self.last_unique_id + 1, isproxy, isproxytarget, proxyaddress, num, maxplayers, bots, gamedir, map, os, password, dedicated, secure, info, len(info))
            server_manager.add_server(server)
        else:
    ...
    command_handlers = {b"0":packet_heartbeat2, b'q':packet_getchallenge, b'a':packet_heartbeat, b'b':packet_shutdown, b'c':packet_get_servers, b'e':packet_get_batch, b'1':packet_get_batch2, b'g':packet_get_motd, b'i':packet_ping, b'k':packet_nack, }
