...
class MasterServer(UDPNetworkHandler):
    ...
    def packet_heartbeat(self, data, address):
        ...
        server = server_manager.get_server_by_address(address)
        if not server:
            self.last_unique_id += 1
            server = Server(address, False, False, int(time.time()), self.last_unique_id+1, False, False, "", int(num), 0, 0,
    ...
    def packet_heartbeat2(self, data, address):
        ...
        server = server_manager.is_server_in_list(address)
        if not server:
            self.last_unique_id += 1
            server = Server(address, False, False, 0, self.last_unique_id+1, isproxy, isproxytarget, proxyaddress,
    ...
    def packet_get_servers_batch_responder(self, address, truenextid, criteria) :
        ...
        server_list = server_manager.get_all_servers(criteria)
    ...
    def packet_get_batch2(self, data, address):
        ...
        print(repr(info))
        if info:
            criteria = server_manager.parse_criteria_from_info_client(info)
        print(repr(criteria))
