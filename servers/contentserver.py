...
class contentserver(TCPNetworkHandler):
    ...
    def __init__(self, port, config):
        ...
        if not globalvars.aio_server:
            ...
            #server_ip = real_socket.inet_aton(server_ip)
        ...
        if len(msg) == 0:
            ...
            while True:
                ...
                if command[0:1] == b"\x00" : #SEND MANIFEST AND PROCESS RESPONSE
                    ...
                    connid |= 0x80000000
                    key = b"\x69" * 0x10
                    if utilities.encryption.verify_message(key, command[9:]):
                        log.debug(clientid + repr(utilities.encryption.verify_message(key, command[9:])))
                    ...
                    while True:
                        ...
                        if command[0:1] == b"\x02" : #SEND MANIFEST AGAIN
                            ...
                            (storageid, messageid, fileid, filepart, numparts, dummy2) = struct.unpack(">xLLLLLB",
                                                                                                       command)
            ...
            while True:
                ...
                if command[0:1] == b"\x00":  # BANNER
                    ...
                    if globalvars.steamui_ver < 24:
                        ...
                        log.debug(b"Checksum fixed from " + binascii.b2a_hex(oldchecksum) + b" to " + binascii.b2a_hex(
                            checksum))
                    ...
                    log.debug(
                        b"Checksum fixed from " + binascii.b2a_hex(oldchecksum) + b" to " + binascii.b2a_hex(checksum))
            ...
            while True:
                ...
                if command[0:1] == b"\x00":  # BANNER
                    ...
                    if checksum == command[1:]:
                        ...
                        log.debug(clientid + "We validate it: " + repr(binascii.b2a_hex(command)))

                        client_socket.send(b"\x00\x00\x00\x00")

                    else:
                        log.info(clientid + "Client didn't match our checksum for secondblob")
                        log.debug(clientid + "Sending new blob: " + repr(binascii.b2a_hex(command)))
