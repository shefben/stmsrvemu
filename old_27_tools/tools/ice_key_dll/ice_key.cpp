...
unsigned char* readBinaryFile(const char* filename, size_t* fileSize) {
	...
	// Get the file size by seeking to the end of the file and telling the position
...
extern "C" __declspec(dllexport) char * cser_decrypt(unsigned char * data)
{
	char CSER_ICE_KEY_q[8]={0x36,0xAF,0xA5,0x05,0x4C,0xFB,0x1D,0x71};

	char CSER_ICE_KEY_g[8]={0x1B,0xC8,0x0D,0x0E,0x53,0x2D,0xB8,0x36};
	//printf("\nrecieved: %s\n\n", data);
	size_t fileSize;
	unsigned char* buffer = readBinaryFile(".\clientstats\192.168.3.180.20230917-083358.hwsurvey.txt", &fileSize);

	// cser header/packet details
	//		u8(C2M_UPLOADDATA_PROTOCOL_VERSION)
	//		u16(encryptedlength)
	//		remainder=encrypteddata

	WORD len=fileSize;
	char * sentData=(char*)buffer;
	char * decrypted=(char *)malloc(len);

	IceKey ice(1);
	ice.set((unsigned char *)(CSER_ICE_KEY_g));
	for (int ind=0;ind<173/8;ind++)
	{
		ice.decrypt((unsigned char *)(sentData+8*ind),(unsigned char *)(decrypted+8*ind));
	}
	printf("decrypted: %s\n\n", decrypted);
	return decrypted;
	//free(decrypted);
...
extern "C" __declspec(dllexport)   unsigned char* pkt_decrypt(char * sentData, WORD len, unsigned char* key)
{
	...
	IceKey ice(1);
	ice.set((unsigned char *)(key));
	for (int ind=0;ind<len/8;ind++)
	{
		ice.decrypt((unsigned char *)(sentData+8*ind),(unsigned char *)(decrypted+8*ind));
	}

	return (unsigned char*)decrypted;
	//free(decrypted);
}