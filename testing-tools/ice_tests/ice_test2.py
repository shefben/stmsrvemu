...
if __name__ == "__main__":
    key = r"\x1B\xC8\x0D\x0E\x53\x2D\xB8\x36"  # 16 bytes key
    ...
    chunk_size = 8
    plaintext_chunks = split_into_chunks(plaintext.encode('utf-8'), chunk_size)

    encrypted_chunks = encrypt_data(ice, plaintext_chunks)
    decrypted_chunks = decrypt_data(ice, encrypted_chunks)

    encrypted_result = ''.join(str(chunk) for chunk in encrypted_chunks)
    decrypted_result = ''.join(str(chunk) for chunk in decrypted_chunks).rstrip('\x00')
