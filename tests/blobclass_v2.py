...
class UserRegistry:
    ...
    class BlobData:
        ...
        def to_blob(self, k_v1, k):
            blob = {
                UserRegistry.VERSION_NUM: struct.pack("<H", 1),
                UserRegistry.UNIQUE_ACCOUNT_NAME: self.username + b"\x00",
                UserRegistry.ACCOUNT_CREATION_TIME: self.utils.unixtime_to_steamtime(self.createtime),
                UserRegistry.OPTIONAL_ACCOUNT_CREATION_KEY: self.accountkey + b"\x00",
                # Skipping OptionalBillingInfoRecord and AccountUserPasswordsRecord as they are not included
            }

            # AccountUsersRecord
            entry = {
                b"\x01\x00\x00\x00": k_v1(self.userid) if self.utils.version == 1 else k(self.userid),
                b"\x02\x00\x00\x00": struct.pack("<H", 1),
                b"\x03\x00\x00\x00": {}
            }
            ...
            for _, subid, subtime in self.subrows:
                entry = {
                    b"\x01\x00\x00\x00": self.utils.unixtime_to_steamtime(subtime),
    ...
    def parse_blob(blob):
        ...
        parsed_data = {
            'version_num': struct.unpack("<H", blob[UserRegistry.VERSION_NUM])[0],
            'unique_account_name': blob[UserRegistry.UNIQUE_ACCOUNT_NAME].decode().rstrip('\x00'),
...
user_registry_data = UserRegistry.BlobData(
    username=b"user123",
    createtime=1620000000,
    accountkey=b"key123",
    userid=12345,
    subrows=[(None, 1001, 1620000000)],  # Example data for subrows
    CDR={UserRegistry.ACCOUNT_CREATION_TIME: {struct.pack('<I', 1001): {UserRegistry.ACCOUNT_USERS_RECORD: [b'\x01\x02\x03\x04']}}},
    cellid=1001,
    utils=utils,  # Utility functions, including 'unixtime_to_steamtime' and 'version'
