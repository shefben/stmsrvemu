...
class DatabaseDriver(BaseDatabaseDriver) :
    ...
    def connect(self, connection_string) :
        with self.lock :
            ...
            """tablename_list = ("FirstBlob", "accountexternalbillinginforecord", "accountpaymentcardinforecord", "accountprepurchasedinforecord",
                              "accountsubscriptionsbillinginforecord", "accountsubscriptionsrecord", "ccardtype",  "custsupportaccountrecord",
                              "derivedsubscribedappsrecord", "serversettings", "steamapplications", "steamsub_apps","subscriptionstatus",
                              "steamsubscriptions",  "userappaccessrightsrecord", "userregistry", "vacrecord")

            for table_name in tablename_list :
                if not self.check_table_exists(table_name) :
                    self.create_firstblob_table( )"""
    ...
    def select_data(self, table, where_clause=None) :
        sel = select([table]).where(where_clause) if where_clause else select([table])
        return self.execute_query(sel)

    def update_data(self, table, where_clause, new_values) :
        upd = table.update( ).where(where_clause).values(**new_values)
        self.execute_query(upd)

    def remove_data(self, table, where_clause) :
        rem = table.delete( ).where(where_clause)
        self.execute_query(rem)

    def get_rows_by_date(self, table, date_column, order='asc') :
