...
class AuthDatabase(object) :
    ...
    def create_user(self, username, salted_answer_to_question_digest,
                    passphrase_salt, answer_to_question_salt, personal_question,
                    salted_passphrase_digest, account_email_address) :
        ...
        expanded_numbers = []
        ranges = [
            "1-5", "11-25", "30-35", "40-45", "50-51", "56", "60-65", "70",
            "72-79", "81-89", "92", "104-111", "204", "205", "242-253", "0",
            "260", "80", "95", "100", "101-103", "200", "210", "241"
        ]
        for r in ranges :
            if "-" in r :
        ...
        data = {
            ...
            'SteamLocalUserID'                    : self.steam_id,
            'UserType'                            : 1,
            'SaltedAnswerToQuestionDigest'        : binascii.b2a_hex(salted_answer_to_question_digest),
            'PassphraseSalt'                      : binascii.b2a_hex(passphrase_salt),
            'AnswerToQuestionSalt'                : binascii.b2a_hex(answer_to_question_salt),
            'PersonalQuestion'                    : personal_question,
            'SaltedPassphraseDigest'              : binascii.b2a_hex(salted_passphrase_digest),
        ...
        try :
            self.db.insert_data('userregistry', data)
        ...
        data = {
            ...
            'UserRegistry_UniqueID'     : self.unique_id,
            'SubscriptionID'            : '0',
            'SubscribedDate'            : accountcreationtime,
            'UnsubscribedDate'          : utils.add_100yrs(accountcreationtime),
        ...
        next_unique_billing_id = self.db.get_next_id(
    ...
    def insert_subscription(self,
        ...
        data = {
            ...
            'UnsubscribedDate'          : utils.add_100yrs(current_time),
            'SubscriptionStatus'        : '1',  # Set the subscription status value here
            'StatusChangeFlag'          : '0',  # Set the status change flag value here
            'PreviousSubscriptionState' : '0'  # Set the previous subscription state value here
        }
        try :
            ...
            # Handle the exception here (e.g., print an error message)
            log.error(
                    "(SQLError)[Insert Sub] Error occurred while inserting into accountsubscriptionsrecord:",
                    e)
            return -1

        if paymenttype == 6 :
            next_unique_prepurchase_id = 0
            next_unique_prepurchase_id = self.db.get_next_id(
                    'accountprepurchasedinforecord', 'UniqueID')

            (TypeOfProofOfPurchase,
             BinaryProofOfPurchaseToken) = tuple_paymentrecord
            ...
            try :
                ...
                # Handle the exception here (e.g., print an error message)
                log.error(
                        "(SQLError)[Insert Sub] Error occurred while inserting into accountprepurchasedinforecord:",
                        e)
                return -1
        elif paymenttype == 5 :
            next_unique_paymentcard_recordID = 0
            next_unique_paymentcard_recordID = self.db.get_next_id(
                    'accountpaymentcardinforecord', 'UniqueID')

            (PaymentCardType, CardNumber, CardHolderName, CardExpYear,
             CardExpMonth, CardCVV2, BillingAddress1, BillingAddress2,
             BillingCity, BillinZip, BillingState, BillingCountry,
             BillingPhone, BillinEmailAddress, PriceBeforeTax,
             TaxAmount) = tuple_paymentrecord
            ...
            try :
                ...
                # Handle the exception here (e.g., print an error message)
                log.error(
                        "(SQLError)[Insert Sub] Error occurred while inserting into accountpaymentcardinforecord:",
                        e)
                return -1
        else :  # Payment id must either be 7 or unknown
            if paymenttype != 7 :
                log.error(username +
                          "Has an invalid payment type id! PaymentTypeID: " +
                          str(paymenttype))
            pass

        paymenttype = 5 if paymenttype == 1 else (
            6 if paymenttype == 2 else 7
        )  # Set the result id (4-7) instead of using the request id (1-3)
        ...
        try :
            ...
            return -1
        log.info(
                "Successfully inserted new Subscription To Database, Subscription ID: "
                + str(SubscriptionID) + " Username: " + username)
    ...
    def get_user_dictionary(self, username) :
        ...
        userrecord_dbdata = self.db.select_data(
        ...
        if len(userrecord_dbdata) > 1 :
            ...
            user_registry_builder.add_entry(
                    b"\x00\x00\x00\x00",
                    utils.to_hex_16bit(userrecord_dbdata_variables['subscriptionrecord_version'])
            )
            user_registry_builder.add_entry(
                    b"\x01\x00\x00\x00",
                    bytes(userrecord_dbdata_variables['UniqueUserName'] + b"\x00")
            )
            user_registry_builder.add_entry(
                    b"\x02\x00\x00\x00",
                    str(utils.datetime_to_steamtime(userrecord_dbdata_variables['AccountCreationTime']))
            )
            user_registry_builder.add_entry(
                    b"\x03\x00\x00\x00",
                    str(userrecord_dbdata_variables['OptionalAccountCreationKey'] + b'\x00')
            )

            Numbers = str(userrecord_dbdata_variables['DerivedSubscribedAppsRecord'])
            numbers_list = Numbers.split(',')
            sub_dict = {
                utils.to_hex_32bit(number) : ''
            ...
            user_registry_builder.add_entry(
                    b"\x09\x00\x00\x00",
                    str(utils.datetime_to_steamtime(userrecord_dbdata_variables['LastRecalcDerivedSubscribedAppsTime']))
            )
            user_registry_builder.add_entry(
                    b"\x0a\x00\x00\x00",
                    str(utils.to_hex_32bit(userrecord_dbdata_variables['Cellid']))
            )
            user_registry_builder.add_entry(
                    b"\x0b\x00\x00\x00",
                    str(userrecord_dbdata_variables['AccountEmailAddress'] + b"\x00")
            )
            user_registry_builder.add_entry(
                    b"\x0e\x00\x00\x00",
                    str(utils.datetime_to_steamtime(userrecord_dbdata_variables['AccountLastModifiedTime']))
            )
            ...
            steam_local_user_id = userrecord_dbdata_variables[
                'SteamLocalUserID']
            ...
            user_registry_builder.AccountUsersRecord(
                    username.encode("ascii"), {
                        b"\x01\x00\x00\x00" :
                            utils.to_hex_32bit(userrecord_dbdata_variables['SteamLocalUserID']) + b"\x00\x00\x00\x00",
                        b"\x02\x00\x00\x00" :
                            utils.to_hex_16bit(userrecord_dbdata_variables['UserType']), b"\x03\x00\x00\x00" : {},
                    }
            )

            i = 0
            subscriptionsbilling_record_data = self.db.select_data(
                    'accountsubscriptionsbillinginforecord',
                    condition="UserRegistry_UniqueID = '{}'".format(userrecord_dbdata_variables['UniqueID'])
            )
                ...
                while (i < num_subscriptionsbilling_record_rows) :
                    ...
                    if AccountPaymentCardInfoRecord == '7' :
                        user_registry_builder.add_account_subscriptions_billing_info(
                                utils.to_hex_32bit(SubscriptionBillingInfoTypeid),
                                utils.to_hex_8bit(AccountPaymentCardInfoRecord))

                    elif AccountPaymentCardInfoRecord == '6' :
                        subprepurchase_record_data = self.db.select_data(
                                'accountprepurchasedinforecord',
                                condition="UniqueID = '{}'".format(subscriptionsbilling_record_data[i]['AccountPrepurchasedInfoRecord_UniqueID']))
                        user_registry_builder.add_account_subscriptions_billing_info(
                                utils.to_hex_32bit(SubscriptionBillingInfoTypeid),
                                utils.to_hex_8bit(AccountPaymentCardInfoRecord),
                        ...
                        subpaymentcard_record_data = self.db.select_data(
                                'accountpaymentcardinforecord',
                                condition="UniqueID = '{}'".format(subscriptionsbilling_record_data[i]['AccountPaymentCardReceiptRecord_UniqueID']))
                        paymentcard_dbdata_variables = subpaymentcard_record_data[0]

                        user_registry_builder.add_account_subscriptions_billing_info(
                                utils.to_hex_32bit(SubscriptionBillingInfoTypeid),
                                utils.to_hex_8bit(AccountPaymentCardInfoRecord),
                                utils.to_hex_8bit(paymentcard_dbdata_variables['PaymentCardType']),
                                paymentcard_dbdata_variables['CardNumber'] + b'\x00',
                                paymentcard_dbdata_variables['CardHolderName'] + b'\x00',
                                paymentcard_dbdata_variables['CardExpYear'] + b'\x00',
                                paymentcard_dbdata_variables['CardExpMonth'] + b'\x00',
                                paymentcard_dbdata_variables['CardCVV2'] + b'\x00',
                                paymentcard_dbdata_variables['BillingAddress1'] + b'\x00',
                                paymentcard_dbdata_variables['BillingAddress2'] + b'\x00',
                                paymentcard_dbdata_variables['BillingCity'] + b'\x00',
                                paymentcard_dbdata_variables['BillingZip'] + b'\x00',
                                paymentcard_dbdata_variables['BillingState'] + b'\x00',
                                paymentcard_dbdata_variables['BillingCountry'] + b'\x00',
                                paymentcard_dbdata_variables['BillingPhone'] + b'\x00',
                                paymentcard_dbdata_variables['BillinEmailAddress'] + b'\x00',
                                utils.to_hex_16bit(paymentcard_dbdata_variables['PriceBeforeTax']),
            ...
            subscription_record_variables = self.db.select_data(
                    'accountsubscriptionsrecord',
                    condition="UserRegistry_UniqueID = '{}'".format(
                            userrecord_dbdata_variables['UniqueID']))
            ...
            if num_subscription_record_rows > 0 :
                while (i < num_subscription_record_rows) :
                    user_registry_builder.add_account_subscription(
                            utils.to_hex_32bit(
                                    subscription_record_variables[i]
                                    ['SubscriptionID']),
                            str(
                                    utils.datetime_to_steamtime(
                                            subscription_record_variables[i]
                                            ['SubscribedDate'])),
                            str(
                                    utils.datetime_to_steamtime(
    ...
    def insert_subscription_beta1(self, username, SubscriptionID) :
        ...
        next_unique_sub_id = self.db.get_next_id('beta1_subscriptions',
                                                 'UniqueID')
        ...
        data = {
            ...
            'UserRegistry_UniqueID' : UserRegistry_UniqueID,
            'SubscriptionID'        :
                SubscriptionID,  # Set the subscribed date value here
            'SubscribedDate'        :
                current_time,  # Set the unsubscribed date value here
        }
        try :
            self.db.insert_data('accountsubscriptionsrecord', data)
        except Exception as e :
    ...
    def get_user_dictionary_beta1(self, username) :
        ...
        if len(userrecord_dbdata) > 1 :
            ...
            log.error(
                    "More than 1 row returned when loading user from database! user: "
                    + username)
            return 2
        elif len(userrecord_dbdata) > 0 :

            user_registry_builder.add_entry(
                    b"\x00\x00\x00\x00", "\x01\x00")  # Record Version, beta1 = v1
            ...
            user_registry_builder.add_entry(
                    ...
                    str(
                            utils.datetime_to_steamtime(
                                    userrecord_dbdata_variables['AccountCreationTime'])))

            user_registry_builder.add_entry(
                    b"\x03\x00\x00\x00",
                    str(userrecord_dbdata_variables['OptionalAccountCreationKey'] +
                        b'\x00'))

            Numbers = str(
                    userrecord_dbdata_variables['DerivedSubscribedAppsRecord'])
            numbers_list = Numbers.split(',')
            sub_dict = {
                utils.to_hex_32bit(number) : ''
                for number in numbers_list
            ...
            user_registry_builder.add_entry(
                    ...
                    str(
                            utils.to_hex_32bit(
                                    userrecord_dbdata_variables['Cellid'])))
            ...
            user_registry_builder.AccountUsersRecord(
                    username.encode("ascii"), {
                        b"\x01\x00\x00\x00" :
                            utils.to_hex_32bit(
                                    userrecord_dbdata_variables['SteamLocalUserID']),
                        b"\x02\x00\x00\x00" :
                            utils.to_hex_16bit(1),
                        b"\x03\x00\x00\x00" : {},
                    })
            ...
            if num_subscription_record_rows > 0 :
                while i < num_subscription_record_rows :
                    user_registry_builder.add_account_subscription_beta1(
                            utils.to_hex_32bit(
                                    subscription_record_variables[i]
                                    ['SubscriptionID']),
                            str(
            ...
            log.error(
                    "(SQLError)[Get_Dictionary] User Does Not Exist!  Should Not Have Gotten This Far!"
            )
    ...
    def update_subscription_records(self, username, status, flag, state) :
        try :
            ...
            user_result = self.db.execute_query_with_result(
            ...
            log.error(
                    "(SQLError)[Update_Subscription] Error updating subscription records:"
            )
    ...
    def generate_and_insert_verification_code(self, username) :
        try :
            ...
            update_query = text(
                    "UPDATE userregistry SET email_verificationcode = :verification_code WHERE UniqueUserName = :username"
            )
            ...
            select_query = text(
                    "SELECT AccountEmailAddress, PersonalQuestion FROM userregistry WHERE UniqueUserName = :username"
            )

            result = self.db.session.execute(select_query, {
                "username" : username
            }).fetchone( )
            ...
            if result :
                return {
                    "AccountEmailAddress" : result[0],
                    "PersonalQuestion"    : result[1],
            ...
            log.error(
                    "(SQLError)[insert Verification] Error when inserting generated verification code or retreiving user email."
            )
            return None
    ...
    def reset_verification_code(self, username) :
        try :
            ...
            update_query = text(
                    "UPDATE userregistry SET email_verificationcode = NULL WHERE UniqueUserName = :username"
            )
    ...
    def check_username(self, username, namecheck=0) :
        rows = self.db.select_data('userregistry', '*',
    ...
    def check_userpw(self, username) :
        user_registry_data = self.db.select_data(
                'userregistry', condition="UniqueUserName = '{}'".format(username))
    ...
    def get_userpass_stuff(self, username) :
        user_registry_data = self.db.select_data(
                'userregistry', condition="UniqueUserName = '{}'".format(username))
        if len(user_registry_data) > 0 :
            return user_registry_data[0][
                'SaltedPassphraseDigest'], user_registry_data[0][
                'PassphraseSalt']
        else :
            return 0
    ...
    def check_resetemail(self, email) :
        user_registry_data = self.db.select_data(
                'userregistry',
                condition="AccountEmailAddress = '{}'".format(email))
        if len(user_registry_data) > 0 :
            return True
        else :
            return False
    ...
    def check_resetcdkey(self, cdkey) :
        checkkey_dbdata = self.db.select_data(
                'accountprepurchasedinforecord',
                condition="BinaryProofOfPurchaseToken = '{}'".format(cdkey))
        if checkkey_dbdata :
            return True
        else :
            return False
    ...
    def change_question(self, username, question, saltedanswer,
                        answertoquestionsalt) :
        # Dictionary containing data to be updated
        data = {
            'PersonalQuestion'             : question,
            'SaltedAnswerToQuestionDigest' : saltedanswer,
            'AnswerToQuestionSalt'         : answertoquestionsalt
        }
