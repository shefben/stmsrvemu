...
class beta1_dbdriver(object):
    ...
    def insert_user(self, username, createtime, accountkey, salt, hash):
        ...
        user_data = {"username":username, "createtime":createtime, "accountkey":accountkey, "salt":salt, "hash":hash}
        # Insert the data
        self.db_driver.insert_data(BaseDatabaseDriver.Beta1_User, user_data)

    def insert_subscription(self, username, subid, subtime):
        # Prepare the data to insert
        subscription_data = {"username":username, "subid":subid, "subtime":subtime}
        # Insert the data
    ...
    def remove_subscription(self, username, subid):
        unsubscribe_data = {"username":username, "subid":subid, }
    ...
    def edit_subscription(self, username, subid, subtime = 0, remove_sub = False):
        ...
        if remove_sub:
            ...
            self.insert_subscription(username, subid, subtime)  # self.db_driver.execute_query(f"INSERT INTO beta1_subscriptions VALUES ( CAST('{username}' AS BLOB), {subid}, {subtime})")
    ...
    def create_user(self, binblob, version):
        ...
        if self.get_user(username) is not None:
            ...
            return False  # TODO BEN send suggested usernames?
    ...
    def get_user_blob(self, username, CDR, version):
        subrows = self.db_driver.select_data(BaseDatabaseDriver.Beta1_Subscriptions, where_clause = BaseDatabaseDriver.Beta1_Subscriptions.username == username)
        print(subrows)
        row = self.get_user(username)
        if row is None:
            return None
