...
def generate_config_lines(ip, user, passw, ip_range):
    return bytes("\n".join([f"CAServerIP = {ip}", "ExitSteamAfterGame = true", "AllowUserLogin = false", "AllowCafeLogin = true", f"MasterServerIP = {ip}", f"MasterLogin = {user}", f"MasterPass = {passw}", f"IPRange1 = {ip_range}", "EnableTimedUpdates = disable", "UpdateStart = 2200", "UpdateEnd = 0200"]), "latin-1")
