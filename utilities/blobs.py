...
class BlobBuilder(object):
    ...
    def to_bytes(self, item):
        if isinstance(item, str):
            ...
            return {self.to_bytes(key):self.to_bytes(value) for key, value in item.items()}
...
def blob_serialize(blobdict):
    ...
    for (name, data) in blobdict.items():
        ...
        if isinstance(data, str):
            data = data.encode('utf-8')  # Convert string values to bytes using UTF-8 encoding (or the appropriate encoding)
