...
def similar_username_generator(base_username, number_of_usernames):
	...
	prepend_modifiers = [
		...
		"pyro", "frost", "thunder", "drift", "flux", "banned", "handbag",
		...
		"stormy", "wild", "ancient", "legendary", "ghost", "frosty", "phallic",
		"hard"
	]
	...
	append_modifiers = {
		...
		"able", "stein", "knight", "_the_cog", "s_boner"
		}

	# Convert append_modifiers to a list if it's a set
	append_modifiers = list(append_modifiers)
	prepend_modifiers = list(prepend_modifiers)
...
# Old version, probably more efficient
"""def suggested_username_generator(base_username):
	suggestions = []
	prepend_modifiers = [
		"mc", "cool", "star", "pro", "king", "ninja", "ace", "neo", "tech",
		"nova", "sky", "galaxy", "phantom", "Mr_", "elite", "shadow", "cyber",
		...
		"nasty", "ultra", "mega", "super", "hyper", "dark", "chaos", "lucky",
		"quick", "sneaky", "crazy", "furious", "silent", "rapid", "chronic",
		"stormy", "wild", "ancient", "legendary", "ghost", "frosty"
	]

	append_modifiers = {
		"rific", "dream", "_the_womanizer", "transvester",
		"_the_unlikable", "_the_terrible", "orama", "ler", "ing",
		"son",  "_jr",  "ifier", "ny", "y", "ji", "men", "zilla",
		"saures", "ton", "ious", "zen", "wick", "son",
		"leigh", "wood", "smith", "max", "ex", "ley", "inator",
		"able", "stein", "knight", "_the_cog", "erater"
		}
	leet_speak = {'a': '4', 'e': '3', 'i': '1', 'o': '0', 's': '5', 't': '7'}

	def randomize_username(username):
		new_username_chars = list(username)  # Convert string to list of characters

		# Randomly replace characters with leet speak (up to 4 letters)
		if random.choice([True, False]):
			eligible_positions = [i for i, char in enumerate(new_username_chars) if char in leet_speak]
			leet_positions = random.sample(eligible_positions, min(2, len(eligible_positions)))
			for pos in leet_positions:
		...
		new_username = ''.join(new_username_chars)  # Convert back to string

		# Randomly add a number at the end
		if random.choice([True, False]):
			num = str(random.randint(0, 9999))
			new_username += num
	...
	for prepend in prepend_modifiers:
		for append in append_modifiers:
			action = random.choice(['append', 'prepend', 'both', 'none'])
			if action == 'append':
				suggestions.append(randomize_username(base_username + append))
			elif action == 'prepend':
				suggestions.append(randomize_username(prepend + base_username))
			elif action == 'both':
				suggestions.append(randomize_username(prepend + base_username + append))
			elif action == 'none':
				suggestions.append(randomize_username(base_username))
