...
def finalinitialize(log):
    ...
    if config["public_ip"] != "0.0.0.0":
        file = neuter_file(file, config["public_ip"], config["dir_server_port"], b"SteamNew.exe", False)
        file2 = neuter_file(file2, config["server_ip"], config["dir_server_port"], b"SteamNewLAN.exe", True)
    ...
    if globalvars.record_ver != 0 and config["hldsupkg"] != "":
        ...
        file = neuter_file(file, config["public_ip"], config["dir_server_port"], b"HldsUpdateToolNew.exe", False)
        file2 = neuter_file(file, config["server_ip"], config["dir_server_port"], b"HldsUpdateToolNew.exe", True)
