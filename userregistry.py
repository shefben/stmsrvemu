...
class UserRegistryBuilder(object):
    ...
    def __init__(self):
        self.registry = {}
        self.log = logging.getLogger('userRegBuilder')
    ...
    def InsertBaseUserRegistryInformation(self, Version, UniqueUsername, AccountCreationTime,
                                          OptionalAccountCreationKey,
                                          LastRecalcDerivedSubscribedAppsTime, Cellid, AccountEmailAddress,
    ...
    def add_account_subscriptions_billing_info(self,
        ...
        if payment_card_type == b'\x07':
            ...
            self.log.warning("Was Sent an Invalid Payment type! type: {}".format(payment_card_type))
            return
    ...
    def beta1_InsertBaseUserRegistryInformation(
            self, UniqueUsername, AccountCreationTime,
            LastRecalcDerivedSubscribedAppsTime, Cellid):
    ...
    def beta1_InsertAccountUserRecord(self,
                                      UniqueUserName,
                                      SteamLocalUserID,
                                      UserType,
                                      UserAppAccessRightsRecord=''):
        self.AccountUsersRecord(
            UniqueUserName,
            {
                b"\x01\x00\x00\x00": SteamLocalUserID,  # 4 bytes
                b"\x02\x00\x00\x00": b'\x01\x01',  # UserType, 0x01 0x01
                b"\x03\x00\x00\x00": UserAppAccessRightsRecord
                if UserAppAccessRightsRecord != '' else {},
            })

    def add_account_subscription_beta1(self, subscription_id, subscribed_date):
        subscription_entry = {
            b"\x01\x00\x00\x00": subscribed_date,
    ...
    def beta1_DerivedSubscribedAppsRecord(self, value_dict):
        self.add_subdict(b"\x08\x00\x00\x00", "",
                         value_dict)  # b"\x42\x00\x00\x00": b"",
