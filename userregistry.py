...
class UserRegistryBuilder(object):
    ...
    def to_bytes(self, item):
        if isinstance(item, str):
            ...
            return {self.to_bytes(key):self.to_bytes(value) for key, value in list(item.items())}
    ...
    def add_account_subscription_beta1(self, subscription_id, subscribed_date):
        subscription_entry = {b"\x01\x00\x00\x00":subscribed_date, b'\x02\x00\x00\x00':'\x00\x00\x00\x00\x00\x00\x00\x00', }
        self.add_subdict(b"\x07\x00\x00\x00", subscription_id, subscription_entry)
