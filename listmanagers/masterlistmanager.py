...
class Server:
    def __init__(self, address, isoldserver, islan, _time, uniqueid, isproxy, isProxyTarget, proxyTarget, players, maxplayers, bots, gamedir, sv_map, os, password, dedicated, secure, info, info_length):
        self.time = int(0)
...
class MasterServerManager:
    def __init__(self):
        self.servers = []
        self.indexes = {'address':{}, # 'isoldserver': {},
                'islan':          {}, # 'time': {},
                'isproxy':        {}, 'isProxyTarget':{}, # 'proxyTarget': {},
                'players':        {}, 'max':{}, 'gamedir':{}, 'map':{}, 'os':{}, 'password':{}, 'dedicated':{}, 'secure':{}}
    ...
    def parse_criteria_from_info_client(self, info):
        criteria = {}
        criteria_keys = {'map':'string', 'gamedir':'string', 'dedicated':'boolean', 'secure':'boolean', 'full':'boolean', 'empty':'boolean', 'linux':'boolean', 'proxy':'boolean', 'proxytarget':'string'  # Assuming 'proxytarget' is a string
        }
